##########################################################################################
<#
.NAME: RemoveUser-to-MultipleAD_ServerAdminGroups.ps1

.SYNOPSIS 
    Removes the User to Multiple Server AD Groups listed in InputFile

.PARAMETER User   
    PMFKey which needs to be Removeed to the AD Groups
...
.NOTES
    ...
    Run the Script as .\RemoveUser-to-MultipleAD_ServerAdminGroups.ps1 -User pmf
...
if($a){
    ...
    Write-Host ""
    Write-Host "-----------------------------------------------------"
    Write-Host "$user needs to be Removed from $E Groups"
    Write-Host "-----------------------------------------------------"
    Write-Host ""
            ...
            if((Get-ADGroupMember $Group).name -contains $User){
                ...
                try{
                    ...
                    if((Get-ADGroupMember $Group).name -notcontains $User){
                        ...
                        $GroupDetails.Status="Removed Successfully"
                    
                    }else{
                        
                        $GroupDetails.Status="Unable to Remove the User"
                    }
                }
                ...
                Catch{               
                    $GroupDetails.Status="Unable to Remove"
                }
            
            }else{
                
                $GroupDetails.Status="User doesn't have has Access"
    ...
    $OutputFile="C:\Temp\Bulk_ADGroupMemberRemoval_$(get-date -f "yyyy-MM-dd_HH-mm-ss").csv"
