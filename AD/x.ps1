##########################################################################################
<#
.NAME: AddGroup-to-MultipleAD_ServerAdminGroups.ps1
...
if($a){
    ...
    $PMF_key = (Get-ADGroup $User -Properties SamAccountName).SamAccountName
        ...
        $GroupDetails.GroupName=$Group
            ...
            if((Get-ADGroupMember $Group).name -notcontains $User){
                    ...
                    if((Get-ADGroupMember $Group).name -contains $User){
                        ...
                        $GroupDetails.Status="Unable to Add the Group"
                    }
               
            
            }else{
                
                $GroupDetails.Status="Group Already has Access"
