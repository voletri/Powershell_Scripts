...
.Author: salmo07
.Modified on: 18th November 2015
######################################################################################>
...
Function Send-Email ($User){
    ...
    $body ="
        <div STYLE='font-family: Century Gothic; font-size: 14px; color: black'>
            ...
            <p>This is to inform you, that your SG Account <strong>$($User.Name)</strong> password will expire <strong> $messageDays &nbsp;</strong> ( <strong>$("{0:D}" -f $User.ExpiringDate) &nbsp;UTC</strong>)</p>
            <p>Your SG Account Password was last changed on <strong>$("{0:D}" -f $User.PasswordLastSet)&nbsp;UTC</strong>. &nbsp;</p>
...
if($modulecheck){
    ...
    foreach($User in $Userlist){
        ...
        try{
            ...
            $User.ExpiringDate = $($user.PasswordLastSet) + $DefaultmaxPasswordAge
            ...
            if (($($User.DaysToExpire) -ge "0") -and (($($User.DaysToExpire) -lt 3) -or ($($User.DaysToExpire) -eq 7) ))
            {        
                Send-Email $User 
            }       
        }
        Catch{          
            ...
            $User.ExpiringDate="Error Occured"
